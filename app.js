...
window.handleAddToCart = function (item) {
  ...
  } else if (item.id.startsWith('ps') && item.id !== 'ps1') {
    window.updateState({ modalTipoPizza: item });
  } else if (item.id.startsWith('ps') || item.id.startsWith('pd')) {
...
window.selecionarTipoPizza = function (tipo) {
  const item = state.modalTipoPizza;
  if (tipo === 'inteira') {
    window.updateState({ pizzaSelecionada: item, modalTipoPizza: null });
  } else {
    window.updateState({ 
      fluxoDoisSabores: { sabor1: item, sabor2: null }, 
      modalTipoPizza: null 
    });
  }
};

window.selecionarSabor2 = function (sabor2) {
  window.updateState({ 
    fluxoDoisSabores: { ...state.fluxoDoisSabores, sabor2: sabor2 }
  });
};
...
window.adicionarAoCarrinho = function (item, borda) {
  ...
  if (state.fluxoDoisSabores && state.fluxoDoisSabores.sabor2) {
    const { sabor1, sabor2 } = state.fluxoDoisSabores;
    itemFinal = {
      id: `dois_sabores_${sabor1.id}_${sabor2.id}`,
      nome: "Pizza com dois sabores",
      sabor1: sabor1.nome,
      sabor2: sabor2.nome,
      preco: Math.max(sabor1.preco, sabor2.preco),
      isDoisSabores: true,
      image: sabor1.image
    };
  } else {
  ...
  const existing = state.cart.find(p => 
    p.id === itemFinal.id && 
    p.bordaId === borda.id && 
    (!p.isDoisSabores || (p.sabor1 === itemFinal.sabor1 && p.sabor2 === itemFinal.sabor2))
  );

  if (existing) {
    const newCart = state.cart.map(p => p === existing ? { ...p, quantidade: p.quantidade + 1 } : p);
    window.updateState({ cart: newCart, pizzaSelecionada: null, fluxoDoisSabores: null });
  } else {
    const newItem = { ...itemFinal, bordaId: borda.id, nomeBorda: borda.nome, precoBorda: borda.preco, quantidade: 1 };
    window.updateState({ cart: [...state.cart, newItem], pizzaSelecionada: null, fluxoDoisSabores: null });
  }
};
...
window.generateWhatsAppLink = function () {
  ...
  state.cart.forEach(item => {
    let bordaText = item.precoBorda > 0 ? `\nBorda: ${item.nomeBorda}` : (item.id.toString().startsWith('dois_sabores') || item.id.toString().startsWith('ps') || item.id.toString().startsWith('pd') ? `\nBorda: Sem borda` : '');
    let recheioText = item.recheio ? ` - ${item.recheio}` : '';
    
    if (item.isDoisSabores) {
      text += `• ${item.nome}\nSabor 1: ${item.sabor1}\nSabor 2: ${item.sabor2}${bordaText}\n`;
    } else {
      text += `• ${item.nome}${recheioText}${bordaText}\n`;
    }
  });
...
function render() {
  ...
  if (state.modalTipoPizza) {
    modalsHtml += `
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4">
        <div class="bg-surface rounded-2xl w-full max-w-sm p-6 shadow-2xl">
          <h3 class="text-xl font-bold text-white mb-2">Como deseja a sua pizza?</h3>
          <p class="text-gray-400 text-sm mb-6">${state.modalTipoPizza.nome}</p>
          <div class="space-y-3">
            <button onclick="window.selecionarTipoPizza('inteira')" class="w-full bg-surfaceHighlight hover:bg-primary p-4 rounded-xl transition text-white font-bold">Pizza inteira</button>
            <button onclick="window.selecionarTipoPizza('doisSabores')" class="w-full bg-surfaceHighlight hover:bg-primary p-4 rounded-xl transition text-white font-bold">Escolher dois sabores</button>
          </div>
          <button onclick="window.updateState({ modalTipoPizza: null })" class="w-full mt-4 p-4 text-gray-400 font-semibold hover:text-white transition">Cancelar</button>
        </div>
      </div>
    `;
  }
  ...
  if (state.fluxoDoisSabores && !state.fluxoDoisSabores.sabor2) {
    const saboresDisponiveis = window.menuData.pizzasSalgadas.filter(p => p.id !== 'ps1' && p.id !== state.fluxoDoisSabores.sabor1.id);
    const saboresHtml = saboresDisponiveis.map(p => `
      <button onclick="window.selecionarSabor2(window.menuData.pizzasSalgadas.find(i => i.id === '${p.id}'))"
        class="w-full flex justify-between items-center bg-surfaceHighlight hover:bg-primary p-4 rounded-xl transition text-left">
    ...
    modalsHtml += `
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4">
        <div class="bg-surface rounded-2xl w-full max-w-sm p-6 shadow-2xl max-h-[80vh] flex flex-col">
          <div class="p-2">
            <h3 class="text-xl font-bold text-white mb-1">Escolha o Segundo Sabor</h3>
            <p class="text-gray-400 text-sm mb-4">Sabor 1: ${state.fluxoDoisSabores.sabor1.nome}</p>
          </div>
          <div class="space-y-3 overflow-y-auto px-1 flex-1">
            ${saboresHtml}
          </div>
          <button onclick="window.updateState({ fluxoDoisSabores: null })" class="w-full mt-4 p-4 text-gray-400 font-semibold hover:text-white transition">Cancelar</button>
        </div>
      </div>
    `;
  }
  ...
  if (state.fluxoDoisSabores && state.fluxoDoisSabores.sabor2) {
    const { sabor1, sabor2 } = state.fluxoDoisSabores;
    const precoBase = Math.max(sabor1.preco, sabor2.preco);
    const bordasHtml = BORDAS.map((borda, index) => `
      <button onclick="window.adicionarAoCarrinho(null, BORDAS[${index}])"
        class="w-full flex justify-between items-center bg-surfaceHighlight hover:bg-primary p-4 rounded-xl transition text-left">
        <span class="font-semibold text-white">${borda.nome}</span>
        <span class="text-sm">${borda.preco > 0 ? '+ R$ ' + borda.preco.toFixed(2) : 'Grátis'}</span>
      </button>
    ...
    modalsHtml += `
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-80 p-4">
        <div class="bg-surface rounded-2xl w-full max-w-sm p-6 shadow-2xl">
          <h3 class="text-xl font-bold text-white mb-2">Deseja adicionar borda?</h3>
          <p class="text-gray-300 text-xs mb-1">Sabor 1: ${sabor1.nome}</p>
          <p class="text-gray-300 text-xs mb-1">Sabor 2: ${sabor2.nome}</p>
          <p class="text-primary font-bold text-sm mb-6">Preço Base: R$ ${precoBase.toFixed(2)}</p>
          <div class="space-y-3">
            ${bordasHtml}
          </div>
          <button onclick="window.updateState({ fluxoDoisSabores: null })" class="w-full mt-4 p-4 text-gray-400 font-semibold hover:text-white transition">Cancelar</button>
    ...
    if (state.cart.length === 0) {
      ...
      let itemsListHtml = state.cart.map((item, index) => `
        <div class="flex justify-between items-start">
          <div class="flex-1">
            <h4 class="font-semibold text-white leading-tight">${item.nome}${item.recheio ? ' - ' + item.recheio : ''}</h4>
            ${item.isDoisSabores ? `
              <p class="text-xs text-gray-400 mt-1">Sabor 1: ${item.sabor1}</p>
              <p class="text-xs text-gray-400">Sabor 2: ${item.sabor2}</p>
            ` : ''}
            ${item.precoBorda > 0 ? `<p class="text-xs text-primary mt-1">Borda: ${item.nomeBorda} (+R$ 10,00)</p>` : ''}
            <span class="text-sm text-gray-400 block mt-1">R$ ${((item.preco + (item.precoBorda || 0)) * item.quantidade).toFixed(2)}</span>
          </div>
...
document.addEventListener("DOMContentLoaded", function() {
  try {
    ...
    const root = document.getElementById('root');
    if (root) {
      root.innerHTML = '<div style="color:red; background:white; padding:20px; font-family:sans-serif; z-index:99999; position:relative">' + e.stack + '</div>';
    }
    console.error(e);
  }
});
